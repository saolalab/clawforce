...
from clawbot.core.cron.types import CronJob, CronJobState, CronPayload, CronSchedule, CronStore
                    ...
                    jobs.append(
                        CronJob(
                            ...
                            state=CronJobState(
                                ...
                                or state.get("nextRunAtMs"),
                                last_run_at_ms=state.get("last_run_at_ms")
                                or state.get("lastRunAtMs"),
                                last_status=state.get("last_status") or state.get("lastStatus"),
                                last_error=state.get("last_error") or state.get("lastError"),
                            ),
                            created_at_ms=j.get("created_at_ms") or j.get("createdAtMs", 0),
                            updated_at_ms=j.get("updated_at_ms") or j.get("updatedAtMs", 0),
                            delete_after_run=j.get("delete_after_run")
                            or j.get("deleteAfterRun", False),
                        )
                    )
