/**
 * WebSocket server for Python-Node.js Zalo Personal bridge.
 ...
 * Both endpoints require Authorization: Bearer <token> when BRIDGE_TOKEN is set.
 */

import { createServer, IncomingMessage, ServerResponse, Server as HttpServer } from 'http';
import { WebSocketServer, WebSocket } from 'ws';
import { ZaloClient, ZaloInboundMessage } from './zalo.js';
...
export class ZaloBridgeServer {
  ...
  async start(): Promise<void> {
    ...
    console.log(`🌉 Zalo bridge listening on ws://127.0.0.1:${this.port}`);
    if (this.token) console.log('🔒 Token authentication enabled');

    this.zalo = new ZaloClient({
      authDir: this.authDir,
      onMessage: (msg: ZaloInboundMessage) =>
        this.broadcast({
          type: 'message',
          id: msg.id,
          sender: msg.sender,
  ...
  private async handleCommand(cmd: SendCommand): Promise<void> {
    if (cmd.type === 'send' && this.zalo) {
      const threadType = cmd.threadType || 'user';
      await this.zalo.sendMessage(cmd.to, threadType, cmd.text);
    }
  }
