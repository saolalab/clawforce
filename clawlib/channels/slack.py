...
from clawlib.config.schema import SlackConfig
...
class SlackChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not self.config.app_token:
            missing.append("app token (xapp-...)")
        if missing:
            logger.error(
                "Slack {} not configured. Set in agent Channels settings, save, then restart the agent.",
                " and ".join(missing),
            )
            return
    ...
    async def _on_socket_request(
        ...
        if not self._is_allowed(sender_id, chat_id, channel_type):
            logger.info(
                "Slack: ignoring message (policy) channel_type=%s sender=%s chat=%s dm.enabled=%s group_policy=%s",
                channel_type,
                sender_id,
                chat_id,
                self.config.dm.enabled,
        ...
        if channel_type != "im" and not self._should_respond_in_channel(event_type, text, chat_id):
            logger.info(
                "Slack: ignoring channel message (no mention) channel_type=%s chat=%s group_policy=%s",
                channel_type,
                chat_id,
                self.config.group_policy,
            )
        ...
        logger.info(
            "Slack: processing message channel=%s channel_type=%s text=%s",
            chat_id,
            channel_type,
