...
from clawlib.config.schema import TelegramConfig
...
class TelegramChannel(BaseChannel):
    ...
    def __init__(
        ...
        self._media_dir = (workspace or Path(".")) / "media"
    ...
    async def start(self) -> None:
        ...
        # Use local reference to avoid race condition with concurrent stop()
        app = builder.build()
        ...
        # Check if we were stopped during initialization
        if not self._running:
            logger.info("Telegram bot startup cancelled during initialization")
            await app.stop()
            await app.shutdown()
            return

        # Assign to instance only after successful initialization
        self._app = app

        # Get bot info and register command menu
        bot_info = await app.bot.get_me()
        self._bot_username = bot_info.username or ""
        logger.info(f"Telegram bot @{self._bot_username} connected")
    ...
    async def stop(self) -> None:
        ...
        if self._app:
            ...
            try:
                ...
                logger.debug("Telegram shutdown interrupted by cancellation")
            except Exception as exc:
                logger.warning(f"Error during Telegram shutdown: {exc}")
            finally:
                self._app = None
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm Clawbot.\n\n"
    ...
    def _is_bot_mentioned(self, message) -> bool:
        """Check if this bot was @mentioned in the message text or caption entities."""
        if not self._bot_username:
            return False
        lower_username = self._bot_username.lower()
        for entities, text in (
            ...
            for ent in entities:
                if ent.type == "mention":
                    ...
                    if mentioned.lower().lstrip("@") == lower_username:
                        return True
        return False
    ...
    def _strip_bot_mention(self, text: str) -> str:
        ...
        if not text or not self._bot_username:
            return text
        return re.sub(
            rf"@{re.escape(self._bot_username)}\s*", "", text, flags=re.IGNORECASE
        ).strip()
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Text content — strip the bot mention so the agent sees clean input
        if message.text:
            content_parts.append(
                self._strip_bot_mention(message.text) if is_group else message.text
            )
        if message.caption:
            content_parts.append(
                self._strip_bot_mention(message.caption) if is_group else message.caption
            )
        ...
        if media_file and self._app:
            try:
                ...
                if media_type == "voice" or media_type == "audio":
                    try:
                        from clawbot.providers.transcription import GroqTranscriptionProvider
                        ...
                        logger.debug("Transcription provider not available, skipping")
                        content_parts.append(f"[{media_type}: {file_path}]")
                else:
                    content_parts.append(f"[{media_type}: {file_path}]")
