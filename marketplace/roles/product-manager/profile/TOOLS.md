...
2. **Propose tasks** — Write the task list in plain text (title + description + required capability). Ask: "Does this look right? Anything to add or change?"
...
4. **Create plan** — Call `create_plan(name, description)` once. Then call `create_plan_task(plan_id, title, description)` for each task (unassigned).
